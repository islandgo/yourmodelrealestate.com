...
{
  /**
   * Compiling preferences properties
   *
   * @var string
   * @access public
   */
  public $scss_dir, $css_dir, $compile_method, $scssc, $compile_errors, $sourcemaps, $disabled_auto_enqueue;

  /**
   * Set values for Action::properties
   *
   ...
   *
   * @var object scss - instantiate the compiling object.
   * @var array compile_errors - catches errors from compile
   */
  public function __construct($scss_dir, $css_dir, $compile_method, $sourcemaps, $compile_errors, $disabled_auto_enqueue)
  {
    global $scssc;
    ...
    $this->compile_errors = [];
    $this->disabled_auto_enqueue = $disabled_auto_enqueue;
    $scssc = new Compiler();
   ...
   *
   * @return void - Puts successfully compiled css into apporpriate location
   *                   Puts error in 'compile_errors' property@access public
   */
      ...
      if (is_writable($this->css_dir)) {
        foreach (new DirectoryIterator($cache) as $cache_file) {
          if (pathinfo($cache_file->getFilename(), PATHINFO_EXTENSION) == 'css') {
            ...
            // Delete file on successful write
            unlink($cache . $cache_file->getFilename());
          }
        }
      } else {
        $errors = [
  ...
  {
    if (get_option('aios_scss_compiler_development', 'on')) {
      return true;
    }
  ...
  public function enqueue_files($css_folder)
  {
    foreach (new DirectoryIterator($this->css_dir) as $stylesheet) {
      if (pathinfo($stylesheet->getFilename(), PATHINFO_EXTENSION) == 'css') {
        ...
        if (!$this->style_url_enqueued($uri) && $this->disabled_auto_enqueue != 'yes') {
