...
/**
 * Name: Classic Editor
 * Description: Disabled Gutenberg for Post and Page
 */

namespace AiosInitialSetup\App\Modules\ClassicEditor;
...
class Module {
  ...
  public function init_actions()
  {
    ...
    if (is_multisite()) {
      add_action('wpmu_options', [$this, 'network_settings']);
    ...
    if (! $settings['hide-settings-ui']) {
      ...
      if ( $settings['allow-users'] ) {
        ...
        add_action('profile_personal_options', [$this, 'user_settings']);
      }
    }
    ...
    if ($settings['allow-users']) {
      if ($block_editor) {
        add_filter('use_block_editor_for_post', [$this, 'choose_editor'], 100, 2);
      }

      if ($gutenberg) {
        add_filter('gutenberg_can_edit_post', [$this, 'choose_editor'], 100, 2);

        if ( $settings['editor'] === 'classic' ) {
          $this->remove_gutenberg_hooks( 'some' );
        }
      }

      add_filter('get_edit_post_link', [$this, 'get_edit_post_link']);
      add_filter('redirect_post_location', [$this, 'redirect_location']);
      add_action('edit_form_top', [$this, 'add_redirect_helper']);
      add_action('admin_head-edit.php', [$this, 'add_edit_php_inline_style']);

      add_action('edit_form_top', [$this, 'remember_classic_editor']);
      add_filter('block_editor_settings', [$this, 'remember_block_editor'], 10, 2);

      // Post state (edit.php)
      add_filter('display_post_states', [$this, 'add_post_state'], 10, 2);
      // Row actions (edit.php)
      add_filter('page_row_actions', [$this, 'add_edit_links'], 15, 2);
      add_filter('post_row_actions', [$this, 'add_edit_links'], 15, 2);
    ...
    if ($block_editor)
    {
      ...
      add_action('all_admin_notices', [$this, 'notice_after_upgrade']);
   ...
   * @param string $remove
   */
  public function remove_gutenberg_hooks($remove = 'all') {
    ...
    remove_filter('admin_url', 'gutenberg_modify_add_new_button_url' ); // ld
  ...
  private function get_settings($refresh = 'no') {
    /**
    ...
    if (! empty( $this->settings ) && $refresh === 'no') {
  ...
  private function is_classic($post_id = 0) {
    if (! $post_id) {
  ...
  public function register_settings() {
    ...
    register_setting('writing', 'classic-editor-replace', ['sanitize_callback' => [$this, 'validate_option_editor']]);
    register_setting('writing', 'classic-editor-allow-users', ['sanitize_callback' => [$this, 'validate_option_allow_users']]);

    if (function_exists('add_allowed_options')) {
      add_allowed_options(['writing' => ['classic-editor-replace', 'classic-editor-allow-users']]);
    } else {
      add_option_whitelist(['writing' => ['classic-editor-replace', 'classic-editor-allow-users']]);
    }

    $heading_1 = __('Default editor for all users', 'classic-editor');
    $heading_2 = __('Allow users to switch editors', 'classic-editor');

    add_settings_field('classic-editor-1', $heading_1, [$this, 'settings_1'], 'writing');
    add_settings_field('classic-editor-2', $heading_2, [$this, 'settings_2'], 'writing');
  }
  ...
  public function save_user_settings($user_id) {
    if (
      ...
      $editor = $this->validate_option_editor($_POST['classic-editor-replace']);
   ...
   * Validate
   */
  public function validate_option_editor($value) {
    return $value === 'block' ? 'block' : 'classic';
  }
   ...
   * @return string
   */
  public function validate_option_allow_users($value) {
    return $value === 'allow' ? 'allow' : 'disallow';
  }

  /**
   *
   */
  public function settings_1() {
    $settings = $this->get_settings('refresh');
    ?>
  ...
  public function user_settings() {
    ...
    $settings = $this->get_settings('update');
   ...
   *
   */
  public function notice_after_upgrade() {
    global $pagenow;
    $settings = $this->get_settings();
   ...
   */
  public function add_redirect_helper() {
    ?>
   ...
   * @param $post
   */
  public function remember_classic_editor($post) {
    $post_type = get_post_type($post);

    if ( $post_type && post_type_supports( $post_type, 'editor' ) ) {
      $this->remember( $post->ID, 'classic-editor' );
    }
  }
   ...
   * @return mixed
   */
  public function remember_block_editor($editor_settings, $post) {
    $post_type = get_post_type($post);

    if ( $post_type && $this->can_edit_post_type( $post_type ) ) {
      $this->remember( $post->ID, 'block-editor' );
    }

    return $editor_settings;
  }

  private function remember( $post_id, $editor ) {
   ...
   */
  public function choose_editor( $use_block_editor, $post ) {
    $settings = $this->get_settings();
    $editors = $this->get_enabled_editors_for_post( $post );
    ...
    } elseif ( $this->is_classic( $post->ID ) ) {
      $use_block_editor = false;
    }
   ...
   * @return string
   */
  public function redirect_location( $location ) {
    if (
  ...
  public function enqueue_block_editor_scripts() {
    $editors = $this->get_enabled_editors_for_post( $GLOBALS['post'] );
    ...
    wp_localize_script(
      ...
      ['linkText' => __( 'Switch to Classic Editor', 'classic-editor' )]
    );
  }
  ...
  private function can_edit_post_type( $post_type ) {
    $can_edit = false;
   ...
   */
  private function get_enabled_editors_for_post_type( $post_type ) {
    if ( isset( $this->supported_post_types[ $post_type ] ) ) {
      return $this->supported_post_types[ $post_type ];
    }

    $classic_editor = post_type_supports( $post_type, 'editor' );
    $block_editor = $this->can_edit_post_type( $post_type );
    ...
    $this->supported_post_types[ $post_type ] = $editors;
   ...
   */
  private function get_enabled_editors_for_post( $post ) {
    $post_type = get_post_type( $post );
    ...
    $editors = $this->get_enabled_editors_for_post_type( $post_type );
  ...
  public function add_edit_php_inline_style() {
    ?>
    <style>
  ...
  public function on_admin_init() {
    ...
    if ($post_id && ($settings['editor'] === 'classic' || $this->is_classic($post_id))) {
